...
homo_apo_OGs_acc<-inner_join(hblast[,1:2],meltedOG,by="seqid")
nrow(homo_apo_OGs_acc) #78

unique(homo_apo_OGs_acc$OG) #57 unique OGs with symbols
...
```{r collateGeneSymbols}
apo_OGs_acc_sym<-apo_OGs_acc_sym[order(apo_OGs_acc_sym$symbol),]
apo_OGs_acc_sym<-apo_OGs_acc_sym[!(duplicated(apo_OGs_acc_sym$gene_acc)),]
homo_apo_OGs_acc<-homo_apo_OGs_acc[order(homo_apo_OGs_acc$gene_acc),]


xref_homo2<-full_join(apo_OGs_acc_sym,homo_apo_OGs_acc,by=c("seqid","OG"))
xref_homo2<-xref_homo2[order(xref_homo2$OG),]

names(xref_homo2)<-c("gene_accGG","Homo_seqid","OG","GeneGlobe_symbol","KEGG_geneacc")
KEGG_symbol<-unlist(lapply(strsplit(xref_homo2$KEGG_geneacc,"_"),function(x) x[1]))
xref_homo2<-cbind(xref_homo2,KEGG_symbol)
xref_homo2<-xref_homo2[order(xref_homo2$GeneGlobe_symbol),c(1:4,6)]


names(apo_OGs_acc_sym)<-c("GGacc","seqid","OG","symbol")
symbol<-unlist(lapply(strsplit(homo_apo_OGs_acc$gene_acc,"_"),function(x) x[1]))
homo_apo_OGs_acc2<-cbind(homo_apo_OGs_acc,symbol)
names(homo_apo_OGs_acc2)<-c("KEGGacc","seqid","OG","symbol")
xref_homo2<-full_join(apo_OGs_acc_sym,homo_apo_OGs_acc2,by=c("seqid","OG","symbol"))
xref_homo2<-xref_homo2[order(xref_homo2$OG),]

```

#### Fly apoptosis gene-set, blasted for *Drosophila* hits
```{r dros}
drosblast<-read.table("inputs/Drosophila_KEGG_blastout",header=T,stringsAsFactors = F)
names(drosblast)[1:2]<-c("gene_acc","seqid")
gene_acc_simple<-unlist(strsplit(drosblast$gene_acc,split="[.]"))[c(T,F)]
drosblast$gene_acc<-gene_acc_simple

dros_apo_OGs_acc<-inner_join(drosblast[,1:2],meltedOG,by="seqid")

dros_apo_OGs_acc<-dros_apo_OGs_acc[order(dros_apo_OGs_acc$OG),]
z <- dros_apo_OGs_acc[!duplicated(dros_apo_OGs_acc$OG),]

```

There are `r length(unique(dros_apo_OGs_acc$OG))` unique OGs mapping to `r nrow(dros_apo_OGs_acc)` fly gene symbols.

```{r xref_counts}
# create cross-reference table for symbols, OGs, seq accessions
#merge Drosophila OG/symbol table with GeneGLobe/Kegg table
names(dros_apo_OGs_acc)<-c("symbol_fly","Dmel_id","OG")
xref3<-full_join(xref_homo2,dros_apo_OGs_acc,by="OG")
xref3<-xref3[order(xref3$OG),]
#sort OG rows so that Dmel-apoptosis genes separated from Kegg/GeneGlobe
xref3<-xref3[order(xref3$symbol,xref3$symbol_fly),]


#import OrthoFinder counts (All genes)
#system("head Orthogroups.GeneCount_3_19.csv")
all_OG_counts<-read.table("inputs/Orthogroups.GeneCount_3_19.csv", header=T, stringsAsFactors = F,row.names=1) #25698 genes
...
apo_gene_counts<-all_OG_counts[rownames(all_OG_counts) %in% unique(xref3$OG),]
apo_gene_counts<-apo_gene_counts[order(rownames(apo_gene_counts)),]

#re-label OGs based on human gene symbol or else fly symbol
z <- xref3[!duplicated(xref3$OG),]
z<-z[order(z$OG),]

flylines<-xref3[is.na(xref3$symbol),]
#populate ermpty slots in xref3$symbol with fly symbols
symbol<-c()
for (i in flylines$OG){symbol<-c(symbol,ifelse(is.na(z$symbol[z$OG==i]),z$symbol_fly[z$OG==i],z$symbol[z$OG==i]))}
#fix long symbol names from Dmel
symbol<-unlist(lapply(strsplit(symbol,"_"),function(x) x[1]))

flyxref<-data.frame(cbind(flylines$OG,symbol,flylines$symbol_fly),stringsAsFactors = F); colnames(flyxref)[1]<-"OG"

  for (i in flyxref$OG){

  xref3$symbol[xref3$OG==i]<-ifelse(is.na(xref3$symbol[xref3$OG==i]),flyxref$symbol[flyxref$OG==i],xref3$symbol[xref3$OG==i])
  }

apo_counts_symbol<-apo_gene_counts; 

#capture human symbol or else fly symbol
symbol<-c();
for (i in z$OG){symbol<-c(symbol,ifelse(is.na(z$symbol[z$OG==i]),z$symbol_fly[z$OG==i],z$symbol[z$OG==i]))}
#fix long symbol names from Dmel
symbol<-unlist(lapply(strsplit(symbol,"_"),function(x) x[1]))

rownames(apo_counts_symbol)<-symbol

```

After pooling GeneGlobe, KEGG and Fly apoptosis gene sets, there are `r length(unique(xref3$OG))` unique OGs mapping to `r nrow(xref3)` human and/or fly gene symbols.


```{r duplicates, include=TRUE}
#### Apoptosis gene symbols with shared OrthoGroups membership:
#Human or fly gene symbols sharing membership to a single OrthoFinder cluster:
table<-xref3[duplicated(xref3$OG)==T,]
table[order(table$OG),]
```
...
phylo.heatmap(tree, log(t_chars_reorder+1), length=10, standardize = F)
```
...
ecdy<-extract.clade(tree,node=getMRCA(tree,tip=c("Priapulus","Drosophila")))$tip.label
...
```{r logratio_setup}
#mean_apocounts_non<-apply(bilat.data[is.ecdy==0,],2,FUN=mean)
#mean_apocounts_ecdy<-apply(bilat.data[is.ecdy==1,],2,FUN=mean)
#diff.apo<-mean_apocounts_non-mean_apocounts_ecdy
#logratio.apo<-log(mean_apocounts_non/mean_apocounts_ecdy)
#hist(diff.apo)
#hist(logratio.apo,main="Ratio of Apo genes in non-Ecdysozoa to Ecdysozoa",xlim=c(-5,5))
```
...
#pdf("heatmap.pdf")
phylo.heatmap(tree, log(sig_reorder+1), length=10, standardize = F,fsize=0.5)
#dev.off()
#par(mar=c(5,4,2,2))
sig1a<-sig.data
...
## Ancestral states of GeneGlobe+KEGG Apoptosis genes
### Dollo parsimony ASR
```{r dollo}


#pdf("dolloASR_allGeneGlobe.pdf")
#sigOGs<-t_chars_reorder #used to run all the genes, then saved the table output to csv

#here, only run Dollo parsimony on signficantly enriched genes
pdf("dolloASR_signif1a_bigset.pdf")
library(viridis)
...
{
  #og<-"IGF1R" #use for troubleshooting 
...
## used when running entire ASRs, including non-significant genes
#rownames(dollo.anc.states)<-nodenames
#write.csv(dollo.anc.states,file="allASR.csv")
```
...
par(mar=c(10,5,3,1))
bp<-barplot(c(deut.losses,loph.losses,ecdy.losses,nonscal.losses),ylab="Number of aptoptosis OG losses, inferred by Dollo parsimony",names.arg = c("Deuterostomia","Lophotrochozoa","Ecdysozoa","non-priap"),las=2)
...
library(coin)
#apply(data.frame(loss.table),1,table)
...
par(mar=c(10,5,3,1))
bp<-barplot(matrix(c(deut.losses,loph.losses,ecdy.losses,deut.nonlosses,loph.nonlosses,ecdy.nonlosses),ncol=3,byrow=T),ylab="Number of aptoptosis OG loss events, inferred by Dollo parsimony",names.arg = c("Deuterostomia","Lophotrochozoa","Ecdysozoa"),las=2,ylim=c(0,500))
...
```{r test2}
gsize_fasta1<-read.csv("inputs/genomesizes.csv",row.names = 1)
gsize_fasta<-gsize_fasta1$recip_blast_fastas_cdhit_fastas; names(gsize_fasta)<-rownames(gsize_fasta1)
gsize_fasta<-gsize_fasta[colnames(all_OG_counts)]
gsize_fasta<-gsize_fasta[tr$tip.label]
all_OG_counts<-all_OG_counts[,tr$tip.label]

pear<-round(cor(gsize_fasta,colSums(all_OG_counts)),3)
p<-round(cor.test(gsize_fasta,colSums(all_OG_counts))$p.value,3)

plot(gsize_fasta,colSums(all_OG_counts), main="Correlation CDHIT fasta vs. total assignment to Orthogroup")
legend("topleft",legend=c(paste0("r= ",pear),paste0("p= ",p)),bty="n")
is.ecdy1<-c(rep("non",length(tr$tip.label)-length(ecdy)),rep("ecdysozoa",length(ecdy)))
text(gsize_fasta,colSums(all_OG_counts),names(gsize_fasta),cex=.8,adj=-0.15)

points(gsize_fasta[is.ecdy1=="ecdysozoa"], colSums(all_OG_counts)[is.ecdy1=="ecdysozoa"],pch=16); 

library(ggplot2);library(ggrepel)
ggplot() + aes(x=gsize_fasta,y=colSums(all_OG_counts)) + geom_point() + geom_text_repel(aes(label = names(gsize_fasta)))
...
bilat.nonapo<-bilat.all[!(rownames(bilat.all) %in% apo_OGs_acc_sym$OG), ]
dim(bilat.nonapo)
...
# plot domain states
#phylo with tip labels aligned, NO branch lengths (so tip symbols will align)	
...
#subset OG data to match species for p53data
...
#summary.lm(manova1)$adj.r.squared	#amount of variation in apoptotic gene richness predicted by p53 domain richness, print at end
...
#using ANOVA to estimate signficant predictors+response OGs...will test explictly with PGLS
...
#run ANOVA for each domain and response, no multiple correction because will do it after PGLS
...
#for signfic associations, correst for phylo with pgls;
...
library(gplots)
#to display 3 signif levels
cols<-c("red","pink","black")
my_palette <- colorRampPalette(rev(cols))(n = 299)
# (optional) defines the color breaks manually for a "skewed" color transition
col_breaks = c(seq(0,0.05,length=1),  # for red
  seq(0.051,0.10,length=1),           # for pink
  seq(0.11,1,length=2))             # for black
...
#rearrange heatmap layout
#default lmat=rbind(4:3,2:1) 4-key, 3-col dendro, 2-row dendro, 1-heatmap
#lhei controls height of top elements(key/coldendro), bottom elements(row dendro/heatmap)
...
#FDR correction... doesn't really cut down the number of sig hits
...
```{r pgls_p53presence_all,fig.height=20}
#with binarized data
...
tail(xref3)
xref_homo_pglsP<-left_join(xref3,as.data.frame(tmp),by='symbol')
xref_homo_pglsP<-xref_homo_pglsP[order(xref_homo_pglsP$symbol),]
#duplicated(xref_homo_pglsP$seqid)
#tail(xref_homo_pglsP)
```

```{r prepare_OGsymbolpvalTable3}
...
xref_homo_pglsP2<-left_join(xref_homo_pglsP,fdrs,by='symbol')
write.csv(xref_homo_pglsP2,file="symbols_crossref.domainPGLSpvals_bigdataset.csv")
